...
- **Status**: Completed. `test_demo.py` added with 25 tests covering all three scripts.
...
- **Status**: Documented in `NEURO_SYMBOLIC_DEMO_GUIDE.md` ("Regex NL Layer: Coverage and Limits" section). Hosted API option already described in the "Using a hosted API instead of Ollama" section.
...
- **Status**: Completed. `QUICKSTART.md` added with Path A (code) and Path B (essays).
...
- **Status**: Completed. `.github/workflows/ci.yml` added with Python 3.10/3.11/3.12 matrix.
...
- **Status**: Completed. `requirements.txt` and `SETUP.md` added.
